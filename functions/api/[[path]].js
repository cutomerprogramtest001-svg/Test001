...
export const onRequest = async (ctx) => {
  ...
  const url = new URL(request.url);
    // ==== helpers (ประกาศไว้บนสุด ป้องกัน ReferenceError) ====
  ...
  const send = (data, status = 200, headers = {}) => json(data, status, headers);

  const method = request.method.toUpperCase();
  const db = env.DB; // ต้อง bind D1 เป็น "DB" ใน Pages Settings
